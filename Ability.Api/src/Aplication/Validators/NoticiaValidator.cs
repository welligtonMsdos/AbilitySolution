...
public class NoticiaValidator: AbstractValidator<NoticiaComandDto>
{
	public NoticiaValidator()
	{
        ...
        RuleFor(x => x.Titulo).MaximumLength(50).WithMessage("Título tem que ter no mínimo 50 caracteres");

        RuleFor(x => x.Url).NotEmpty().WithMessage("Url não pode ser vazia");
        RuleFor(x => x.Url).MinimumLength(10).WithMessage("Título tem que ter no mínimo 10 caracteres");
        RuleFor(x => x.Url).MaximumLength(200).WithMessage("Título tem que ter no mínimo 50 caracteres");
    }
}
